...
def constrastLimit(image):
    ...
    yuv_image_eq = cv2.merge(channels)
    
    # Convert the equalized YUV image back to BGR color space
    equalized_image = cv2.cvtColor(yuv_image_eq, cv2.COLOR_YUV2BGR)
...
def findLargestSign(image, contours, threshold, distance_threshold):
    ...
    sign = None
    for c in contours:
        if len(c) > 0:  # Check if contour is not empty
            print("Contour:", c)  # Print contour information
            M = cv2.moments(c.reshape(-1, 1, 2))  # Reshape contour data
            if M["m00"] == 0:
                continue
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            c = c.squeeze(axis=1)  # Remove unnecessary dimensions
            is_sign, distance = contourIsSign(c, [cX, cY], 1 - threshold)
            if is_sign and distance > max_distance and distance > distance_threshold:
                max_distance = distance
                coordinate = c
                left = np.min(coordinate[:, 0])
                top = np.min(coordinate[:, 1])
                right = np.max(coordinate[:, 0])
                bottom = np.max(coordinate[:, 1])
                coordinate = [(left - 2, top - 2), (right + 3, bottom + 1)]
                sign = cropSign(image, coordinate)
    return sign, coordinate
